...
async def create_user_with_note(client, username="planuser", password="secret123", note_title="Daily"):
...
async def test_list_plans_empty(async_client):
    ...
    r = await async_client.get(f"/notes/{note_id}/plans", headers={"Authorization": f"Bearer {token}"})
...
async def test_plans_isolation_between_notes(async_client):
    ...
    r1 = await async_client.get(f"/notes/{note1_id}/plans", headers={"Authorization": f"Bearer {token}"})
    plans1 = r1.json()
    assert len(plans1) == 1
    assert plans1[0]["title"] == "Plan for note 1"
    
    # Note2 should only have its plan
    r2 = await async_client.get(f"/notes/{note2_id}/plans", headers={"Authorization": f"Bearer {token}"})
    plans2 = r2.json()
    assert len(plans2) == 1
